...
public interface ResrvDscnRepository extends JpaRepository<ResrvDscn, Long> {
    // 예약확정된 호스트들의 목록을 구함
    //@Query("SELECT rd FROM ResrvDscn rd WHERE rd.resrvHis.host.hnum IN :#{#host.hnum}")(@Param("host") List<Host> hostList);
    List<ResrvDscn> resrvDscnHostList

    // 요청하는 날짜에 걸친 예약들 중에, 요청인원 > 남은 인원
    // 예약확정 테이블에 없다면 예약가능
    @Query("SELECT rd.resrvHis.host.hnum " +
            "FROM  ResrvDscn rd " +
            "WHERE rd.resrvHis.host.hnum IN :#{#rdHost.resrvHis.host.hnum} " +
