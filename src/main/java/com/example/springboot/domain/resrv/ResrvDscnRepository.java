...
public interface ResrvDscnRepository extends JpaRepository<ResrvDscn, Long> {
    ...
    // 예약확정 테이블에 없다면 예약가능
    @Query("SELECT d.resrvHis.host.hnum " +
            "FROM (" +
            "       SELECT rd " +
            "       FROM ResrvDscn rd " +
            "       WHERE rd.resrvHis.host.hnum " +
            "        IN :#{#host.hnum}" +
            "       ) d " +
            "WHERE :reqPpl > d.restPpl " +
            "AND (d.resrvHis.startDate < :srchEndDate OR d.resrvHis.endDate > :srchStartDate )")
